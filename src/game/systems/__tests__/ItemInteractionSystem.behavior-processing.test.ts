...
describe('ItemInteractionSystem - Behavior Processing', () => {
  ...
  describe('TRANSFORM Behavior', () => {
    it('should transform sprite and deactivate RequiresItem component', () => {
      ...
      (getEntitiesWithComponents as any).mockImplementation(
        (componentTypes: ComponentType[], entitiesArray: any[]) => {
      ...
      const requiresItemComponent = doorEntity.components[
        ComponentType.RequiresItem
      ] as any;
  ...
  describe('REMOVE Behavior', () => {
    it('should remove entity from game world', () => {
      ...
      const hammerEntity = createEntityWithComponents([
        [
          ComponentType.UsableItem,
          { capabilities: ['destroy'], isConsumable: false },
        ],
      ]);
      ...
      const wallEntity = createEntityWithComponents([
        [
          ComponentType.RequiresItem,
          { requiredCapabilities: ['destroy'], isActive: true },
        ],
        [
          ComponentType.InteractionBehavior,
          {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          `Removing entity ${wallEntity.id} from game world`,
        ),
      );
    ...
    it('should spawn contents and remove original entity', () => {
      ...
      (getComponentIfExists as any).mockImplementation(
        (entity: any, componentType: ComponentType) => {
          ...
          if (
            entity === chestEntity &&
            componentType === ComponentType.SpawnContents
          ) {
  ...
  describe('Error Handling', () => {
    it('should handle missing InteractionBehavior component', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'Target entity does not have InteractionBehavior component',
        ),
      );
