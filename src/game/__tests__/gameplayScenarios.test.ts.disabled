...
describe('Core Gameplay Scenarios - Real Jotai Integration', () => {
  ...
  describe('Complete Player Movement Workflow', () => {
    it('should handle complete movement workflow using real Jotai state: input → velocity → position', () => {
      const playerEntity = createTestEntity(
        {
          ...
          sprite: createTestComponent(ComponentType.Sprite, {
            sprite: 'player',
          }),
        },
        'player-1',
      ...
      let playerAfterKeyboard = entitiesAfterKeyboard.find(
        (e) => e.id === 'player-1',
      );
      let velocityAfterKeyboard = getComponentIfExists(
        playerAfterKeyboard!,
      ...
      const updatedUpdateArgs = createTestUpdateArgs(
        entitiesAfterKeyboard,
        gameMap,
      );
